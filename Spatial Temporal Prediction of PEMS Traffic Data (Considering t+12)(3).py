...
# ## Graph-Based Deep Learning for Spatio-Temporal Traffic Forecasting

# ## Why Graph WaveNet?

# Graph WaveNet (GWN) is a deep learning architecture for spatio-temporal forecasting on traffic networks. Unlike traditional models that require a fixed graph of connections between sensors, GWN learns the spatial dependencies adaptively. This means the model can discover hidden connectivity, which is crucial for complex, evolving systems like transportation grids.

# In[ ]:


get_ipython().system('pip install torch torch-geometric==2.4.0 scikit-learn pandas matplotlib seaborn')
...
# ### Graph WaveNet Model Class

# In[ ]:


class GraphWaveNet(nn.Module):
    def __init__(self, num_nodes, in_channels, out_channels, residual_channels=32,
                 skip_channels=256, end_channels=512, num_layers=8, kernel_size=2,
                 dropout=0.3):
        super().__init__()
        self.num_nodes = num_nodes
        self.dropout = dropout
        # Learnable node embeddings for adaptive adjacency
        self.node_emb1 = nn.Parameter(torch.randn(num_nodes, 10), requires_grad=True)
        self.node_emb2 = nn.Parameter(torch.randn(10, num_nodes), requires_grad=True)
        ...
        for dilation in self.dilations:
            self.conv_layers.append(nn.Conv2d(residual_channels, residual_channels,
                                              kernel_size=(1, kernel_size), dilation=(1, dilation)))
            self.skip_layers.append(nn.Conv2d(residual_channels, skip_channels, kernel_size=(1,1)))
    ...
    def forward(self, x):
        ...
        # Adaptive adjacency
        a_adp = F.softmax(F.relu(torch.mm(self.node_emb1, self.node_emb2)), dim=1)
        x = self.start_conv(x)
        skip = 0
        for conv, skip_conn in zip(self.conv_layers, self.skip_layers):
            x_conv = conv(x)
            x_gc = torch.einsum('bcnl,nm->bcml', x_conv, a_adp)
            x = F.relu(x_gc)
            x = F.dropout(x, self.dropout, self.training)
            skip = skip + skip_conn(x)
        out = F.relu(self.end_conv_1(skip))
        out = self.end_conv_2(out)
        return out  # [batch, out_channels, num_nodes, T_out]
...
class TrafficDataset(Dataset):
    def __init__(self, X, y, seq_len):
        self.X = X  # shape [samples, features]
        self.y = y  # shape [samples, out_horizons]
        self.seq_len = seq_len
        self.num_nodes = 1
        if len(X.shape) == 3:  # If [samples, nodes, features]
    ...
    def __getitem__(self, idx):
        if self.num_nodes > 1:
            X_seq = self.X[idx:idx+self.seq_len]   # [seq_len, num_nodes, features]
            X_seq = torch.from_numpy(X_seq).permute(2,1,0)  # [features, num_nodes, seq_len]
        else:
            X_seq = self.X[idx:idx+self.seq_len]  # [seq_len, features]
            X_seq = torch.from_numpy(X_seq).T.unsqueeze(1)  # [features, 1, seq_len]
        y = torch.from_numpy(self.y[idx+self.seq_len]).float()
        return X_seq, y
...
def train_gwn(model, optimizer, criterion, train_loader, val_loader, device, num_epochs=60, patience=6):
    best_val = float('inf')
    best_state = None
    wait = 0
...
def evaluate_gwn(model, data_loader, device):
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
...
# ## Direct Strategy: Training and Evaluation
...
X_np = X_full_scaled.values.astype(np.float32)

for h in HORIZONS:  # [12, 24, 48, 72]
    print(f'\\n=== Training Graph WaveNet Direct: {h}-hour horizon ===')
    for fold, (train_idx, val_idx, test_idx) in enumerate(cv_splits):
        print(f'Fold {fold+1}/{len(cv_splits)}')
        # Prepare data per fold
        X_train = X_np[train_idx]
        y_train = y_full_dict[h].iloc[train_idx].values.astype(np.float32).reshape(-1, 1)
        X_val = X_np[val_idx]
        y_val = y_full_dict[h].iloc[val_idx].values.astype(np.float32).reshape(-1, 1)
        X_test = X_np[test_idx]
        y_test = y_full_dict[h].iloc[test_idx].values.astype(np.float32).reshape(-1, 1)
        
        # Build datasets
        train_ds = TrafficDataset(X_train, y_train, seq_len=SEQ_LEN)
        val_ds = TrafficDataset(X_val, y_val, seq_len=SEQ_LEN)
        test_ds = TrafficDataset(X_test, y_test, seq_len=SEQ_LEN)
        train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True)
        val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False)
        test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=False)
        
        # Model
        model = GraphWaveNet(num_nodes=1, in_channels=len(effective_features), out_channels=1)
        model.to(DEVICE)
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
        criterion = nn.MSELoss()
        model = train_gwn(model, optimizer, criterion, train_loader, val_loader, DEVICE, EPOCHS, PATIENCE)
        
        # Evaluation
        y_true, y_pred = evaluate_gwn(model, test_loader, DEVICE)
        print(f"RMSE: {mean_squared_error(y_true, y_pred, squared=False):.2f}")
        print(f"MAE: {mean_absolute_error(y_true, y_pred):.2f}")
        print(f"R2:  {r2_score(y_true, y_pred):.3f}\n")
        
        # Save model if desired
        torch.save(model.state_dict(), f"gwn_direct_h{h}_fold{fold+1}.pth")


# ## MIMO Strategy (Predict All Horizons Together)

# In[ ]:


# --- Prepare multi-output targets (shape: samples x 4 horizons) ---
y_multi = np.stack([y_full_dict[h].values.astype(np.float32) for h in HORIZONS], axis=1)

print('=== Training Graph WaveNet MIMO (all horizons) ===')
for fold, (train_idx, val_idx, test_idx) in enumerate(cv_splits):
    print(f'Fold {fold+1}/{len(cv_splits)}')
    X_train = X_np[train_idx]
    ...
    # Model
    model = GraphWaveNet(num_nodes=1, in_channels=len(effective_features), out_channels=4)
    model.to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
    criterion = nn.MSELoss()
    model = train_gwn(model, optimizer, criterion, train_loader, val_loader, DEVICE, EPOCHS, PATIENCE)
    
    # Evaluation: output is [N, 4, 1, 1] (batch, out_channels, nodes, 1)
    y_true, y_pred = evaluate_gwn(model, test_loader, DEVICE)
    y_true = y_true.reshape(-1, 4)
    y_pred = y_pred.reshape(-1, 4)
    for i, horizon in enumerate(HORIZONS):
        print(f"Horizon {horizon}h | RMSE: {mean_squared_error(y_true[:,i], y_pred[:,i], squared=False):.2f}")
        print(f"           | MAE:  {mean_absolute_error(y_true[:,i], y_pred[:,i]):.2f}")
        print(f"           | R2:   {r2_score(y_true[:,i], y_pred[:,i]):.3f}\n")
    torch.save(model.state_dict(), f"gwn_mimo_fold{fold+1}.pth")


# ## Hyperparameter Tuning For Graph WaveNet 
...
param_grid = [
    ...
    {'residual_channels': 32, 'skip_channels': 256, 'num_layers': 8, 'dropout': 0.3, 'lr': 0.001},
    # 2. Higher capacity
    {'residual_channels': 64, 'skip_channels': 256, 'num_layers': 8, 'dropout': 0.3, 'lr': 0.001},
    # 3. Shallower, lower dropout
    {'residual_channels': 32, 'skip_channels': 128, 'num_layers': 6, 'dropout': 0.2, 'lr': 0.001},
    # 4. More layers, smaller residual
    {'residual_channels': 16, 'skip_channels': 128, 'num_layers': 10, 'dropout': 0.2, 'lr': 0.0005}
]
...
results = []
# Choose fold and horizon for grid search (e.g., fold=0, horizon='h12')
fold = 0
horizon = 'h12'
train_idx, val_idx, _ = cv_splits[fold]
X_train = X_np[train_idx]
y_train = y_full_dict[horizon].iloc[train_idx].values.astype(np.float32).reshape(-1, 1)
X_val = X_np[val_idx]
y_val = y_full_dict[horizon].iloc[val_idx].values.astype(np.float32).reshape(-1, 1)
train_ds = TrafficDataset(X_train, y_train, seq_len=SEQ_LEN)
val_ds = TrafficDataset(X_val, y_val, seq_len=SEQ_LEN)
train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True)
val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False)
...
for cfg in param_grid:
    ...
    criterion = nn.MSELoss()
    model = train_gwn(model, optimizer, criterion, train_loader, val_loader, DEVICE, EPOCHS, PATIENCE)
    y_true, y_pred = evaluate_gwn(model, val_loader, DEVICE)
    val_rmse = mean_squared_error(y_true, y_pred, squared=False)
    results.append((cfg, val_rmse))
    print(f"Config {cfg}: Validation RMSE = {val_rmse:.2f}")
    if val_rmse < best_val_rmse:
...
# ## Use Best Params for Direct Strategy Training
...
for h in HORIZONS:
    ...
    for fold, (train_idx, val_idx, test_idx) in enumerate(cv_splits):
        ...
        test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=False)
        model = GraphWaveNet(num_nodes=1, in_channels=len(effective_features), out_channels=1,
                             residual_channels=best_config['residual_channels'],
                             skip_channels=best_config['skip_channels'],
...
# ## Use Best Params for MIMO Strategy Training
...
# Aggregate mean scores by model, strategy, and horizon
plot_df = (
    results_df.groupby(['model', 'strategy', 'horizon'])
             .agg({'mae':'mean', 'rmse':'mean', 'r2':'mean'})
             .reset_index()
)
plt.figure(figsize=(11, 6))
sns.barplot(data=plot_df, x='horizon', y='mae', hue='model', ci=None)
plt.title('MAE by Horizon: Direct vs MIMO')
plt.xlabel('Forecast Horizon (hours)')
plt.ylabel('Mean Absolute Error (MAE)')
plt.legend(title='Model')
plt.tight_layout()
plt.savefig('gw_mae_horizon_bar.pdf')
plt.show()


# ## Residual Error Heatmap (Stations × Horizon)

# In[ ]:


# Pivot table: rows=station, columns=horizon, values=mean absolute error
pivot_df = results_df.pivot_table(
    index='station', columns='horizon', values='mae', aggfunc='mean'
)
plt.figure(figsize=(8, 6))
sns.heatmap(pivot_df, cmap='YlGnBu', annot=False)
plt.title('Residuals Across Stations and Horizons')
plt.xlabel('Forecast Horizon (hours)')
plt.ylabel('Station ID')
plt.tight_layout()
plt.savefig('gw_residual_heatmap.pdf')
plt.show()


# ## Learned Adaptive Adjacency Visualization (GWN only)
...
# After training, extract node embeddings and compute adjacency
node_emb1 = model.node_emb1.detach().cpu()
node_emb2 = model.node_emb2.detach().cpu()
adaptive_adj = torch.softmax(torch.relu(torch.matmul(node_emb1, node_emb2)), dim=1).numpy()
plt.figure(figsize=(9, 7))
sns.heatmap(adaptive_adj, cmap='viridis', cbar_kws={'label': 'Connection strength'})
plt.title('Learned Adaptive Adjacency (Graph WaveNet)')
plt.xlabel('Node')
plt.ylabel('Node')
plt.tight_layout()
plt.savefig('gw_learned_adjacency.pdf')
plt.show()


# ## Feature Importance  - GRAVE WAVENET
...
def permutation_importance(model, X, y, base_mae, device, batch_size=64):
    # X: [n_samples, n_features], y: [n_samples, output_dim]
    n_features = X.shape[-1]
    importances = np.zeros(n_features)
    for feat in range(n_features):
        X_perm = X.copy()
        np.random.shuffle(X_perm[:, feat])
        # Prepare data as Tensor and dataloader
        dataset = TrafficDataset(X_perm, y, seq_len=SEQ_LEN)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
        y_true, y_pred = evaluate_gwn(model, loader, device)
        mae = mean_absolute_error(y_true, y_pred)
        importances[feat] = mae - base_mae
    return importances


# ### Feature Importance for Direct

# In[ ]:


# After training your best Graph WaveNet Direct model for one horizon (e.g., 24h)
# X_test, y_test = your test set for the selected fold/horizon

dataset = TrafficDataset(X_test, y_test, seq_len=SEQ_LEN)
test_loader = DataLoader(dataset, batch_size=64, shuffle=False)
y_true, y_pred = evaluate_gwn(direct_model, test_loader, DEVICE)
base_mae = mean_absolute_error(y_true, y_pred)
direct_importances = permutation_importance(direct_model, X_test, y_test, base_mae, DEVICE)

import matplotlib.pyplot as plt
import pandas as pd
imp_df = pd.DataFrame({'feature': effective_features, 'Direct Importance': direct_importances})
imp_df = imp_df.sort_values('Direct Importance', ascending=False)
plt.figure(figsize=(10,5))
plt.barh(imp_df['feature'], imp_df['Direct Importance'], color='steelblue')
plt.title('Feature Importance: Graph WaveNet Direct')
plt.xlabel('MAE Increase (Permutation)')
plt.tight_layout()
plt.savefig('gwn_direct_feat_importance.pdf')
plt.show()


# ## Feature Importance for MIMO

# In[ ]:


# Do the same for your trained MIMO model (outputs for all horizons)

y_true, y_pred = evaluate_gwn(mimo_model, test_loader, DEVICE)
# Combine all horizons (average absolute error over outputs):
base_mae = mean_absolute_error(y_true.flatten(), y_pred.flatten())
mimo_importances = permutation_importance(mimo_model, X_test, y_test, base_mae, DEVICE)

imp_df2 = pd.DataFrame({'feature': effective_features, 'MIMO Importance': mimo_importances})
imp_df2 = imp_df2.sort_values('MIMO Importance', ascending=False)
plt.figure(figsize=(10,5))
plt.barh(imp_df2['feature'], imp_df2['MIMO Importance'], color='tomato', alpha=0.7)
plt.title('Feature Importance: Graph WaveNet MIMO')
plt.xlabel('MAE Increase (Permutation)')
plt.tight_layout()
plt.savefig('gwn_mimo_feat_importance.pdf')
plt.show()


# ## Side-by-Side Comparison Bar Chart ( For the Feature Importance for Direct and MIMO)
...
compare_df = pd.DataFrame({
    ...
    'Direct': direct_importances,
    'MIMO': mimo_importances
})
compare_df = compare_df.set_index('feature').sort_values('Direct', ascending=False)
compare_df.plot.bar(figsize=(12,6))
plt.title('Permutation Feature Importance: Direct vs MIMO')
plt.ylabel('MAE Increase')
plt.xlabel('Feature')
plt.tight_layout()
plt.savefig('gwn_direct_mimo_feat_imp_side_by_side.pdf')
plt.show()
